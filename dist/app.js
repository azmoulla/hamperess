...
let renderTimeout = null; // Used to debounce updateProductView

if ('serviceWorker' in navigator) {
    navigator.serviceWorker.getRegistrations().then(registrations => {
        for (let registration of registrations) {
            registration.unregister();
            console.log("🔴 RED DOT: Service Worker Unregistered to prevent caching.");
        }
    });
}
// Detect touch capability for the Hold-to-Buy logic
    ...
    // Replace the old handler with this one
{
   ...
   { 
        selector: '#back-to-list-detail', 
        handler: () => {
            // Tell the router we are "returning" so it preserves the search
            returnToSearchState = true; 
            router.navigate('/'); 
        } 
    },
    {
...
let dynamicSearchTags = { dietary: [], occasion: [], contents: [] };
...
let currentPriceMax = Infinity; // Needed for the Price Heatmap Slider
...
let returnToSearchState = false; // Flag to remember previous search
let currentTagFilters = []; // Changed to an Array to hold multiple chips


// --- CONFIG: SMART DISCOVERY ENGINE (The Brain) ---
// Maps user intent (keys) to suggested filter chips (values)
let DISCOVERY_CONTEXTS = { 
    'default': { triggers: [], suggestions: ['BESTSELLER'] } 
};
...
async function loadAndDisplayReviews(productId) {
    ...
    container.innerHTML = '<div class="spinner"></div><p style="text-align:center">Loading reviews...</p>';
    ...
    try {
        ...
        if (!Array.isArray(reviews) || reviews.length === 0) {
            container.innerHTML = '<p class="text-gray-500 italic text-center py-4">No reviews yet. Be the first to review this hamper!</p>';
            return;
        }

        let html = '<div class="reviews-list space-y-6">';
        reviews.forEach(review => {
            const stars = generateStarRating(review.rating); // Uses your existing helper
            html += `
                <div class="review-item border-b pb-4 mb-4">
                    ...
                    <div class="mb-2 text-yellow-400 text-sm">${stars}</div>
        ...
        container.innerHTML = '<p class="text-red-500 text-center">Could not load reviews at this time.</p>';
    }
}
// --------------------Fetch Tags------------------- //

// 2. Add this Function to fetch and render tags
// FILE: app.js

// FILE: public/app.js

async function loadDynamicSearchFilters() {
    console.log("Loading dynamic filters...");
    try {
        // FIX: Use the global settings we already fetched instead of calling the API again
        ...
        // Map the tags from the settings object
        dynamicSearchTags.dietary = settings.tags_dietary || [];
        dynamicSearchTags.occasion = settings.tags_occasion || [];
        dynamicSearchTags.contents = settings.tags_contents || [];

        renderFilterGroup('dietary', dynamicSearchTags.dietary);
        renderFilterGroup('occasion', dynamicSearchTags.occasion);
        renderFilterGroup('contents', dynamicSearchTags.contents);

    } catch (error) {
        console.warn("Could not load dynamic filters:", error);
        // Ensure the container exists before trying to set innerHTML
        const container = document.getElementById('filter-container-dietary');
        if (container) container.innerHTML = '<span class="text-xs text-gray-400">No filters available</span>';
    }
}


function renderFilterGroup(category, tags) {
    const container = document.getElementById(`filter-container-${category}`);
    if (!container) return;

    if (!tags || tags.length === 0) {
        container.innerHTML = '<p class="text-sm text-gray-400">No filters available</p>';
        return;
    }

    container.innerHTML = tags.map(tag => `
        <label class="facet-label">
            <input type="checkbox" class="facet-checkbox" data-type="${category}" value="${tag}"> 
            ${tag}
        </label>
    `).join('');

    // Attach listeners so clicking them updates the products immediately
    container.querySelectorAll('.facet-checkbox').forEach(cb => {
...
function initCinematicScroll() {
    console.log("Initializing Cinematic Discovery Engine...");

    const mainHeader = document.querySelector('.main-header');
    const ghostHeader = document.getElementById('ghost-header');
    const vibeTrack = document.getElementById('vibe-track'); // Get the container
    let lastScrollY = window.scrollY;
    let ticking = false;
    const SHOW_GHOST_THRESHOLD = 150; 
    ...
    function updateHeaders(scrollY) {
        if (scrollY <= 10) {
            mainHeader.classList.remove('header-hidden');
            ghostHeader.classList.remove('ghost-visible');
            return;
        }
        if (scrollY > lastScrollY && scrollY > SHOW_GHOST_THRESHOLD) {
            mainHeader.classList.add('header-hidden');
            ghostHeader.classList.remove('ghost-visible');
        } 
        else if (scrollY < lastScrollY && scrollY > SHOW_GHOST_THRESHOLD) {
            mainHeader.classList.add('header-hidden'); 
            ghostHeader.classList.add('ghost-visible'); 
        }
    }
...
if (applyFiltersBtn) applyFiltersBtn.addEventListener('click', () => toggleSidebar(false));
    ...
    window.addEventListener('authchange', () => {
        console.log("setupEventListeners: authchange event caught.");
        handleAuthStateChange();
        
        // Immediate Update: If on Orders page, refresh it now that Auth is ready.
        const ordersPage = document.getElementById('page-my-orders');
        if (ordersPage && ordersPage.style.display !== 'none') {
             console.log("Auth changed while on Orders page. Triggering render...");
             setTimeout(() => renderMyOrdersPage(), 50);
        }
    });

    // --- 🔴 THE CRITICAL FIX: CHECK FOR PRE-EXISTING AUTH ---
    // On Refresh, Firebase often logs in BEFORE these listeners are attached.
    // We check if we are already logged in and manually kickstart the process.
    if (auth && typeof auth.isLoggedIn === 'function' && auth.isLoggedIn()) {
        console.log("setupEventListeners: Auth already active on startup. Triggering manual update.");
        
        // Start fetching user orders/addresses immediately
        handleAuthStateChange(); 

        // If the user refreshed while looking at the orders page, force it to render
        const ordersPage = document.getElementById('page-my-orders');
        if (ordersPage && ordersPage.style.display !== 'none') {
             console.log("setupEventListeners: On orders page at startup. Force rendering...");
             // Small delay to let handleAuthStateChange initialize the data promise
             setTimeout(() => renderMyOrdersPage(), 150);
        }
    }

    // --- 🔴 THE SAFE FIX: ONE-SHOT RESCUE ---
    // Safety net: if after 2 seconds the page is still "stuck" on the loading message, force a fix.
    setTimeout(() => {
        const ordersPage = document.getElementById('page-my-orders');
        if (ordersPage && ordersPage.style.display !== 'none') {
            if (ordersPage.innerText.includes("Loading orders...")) {
                console.warn("Safe Rescue: Page appears stuck after 2s. Triggering recovery.");
                handleAuthStateChange(); 
                renderMyOrdersPage();
            }
        }
    ...
    window.addEventListener('wishlistChange', () => {
        if (pageList.style.display !== 'none') updateProductView();
        if (pageDetail.style.display !== 'none') updateAllWishlistIcons();
        if (document.getElementById('page-wishlist')?.style.display !== 'none') renderWishlistPage();
    });
    ...
    const debouncedSearch = debounce(updateProductView, 300);
    ...
    // --- PASTE THIS INSIDE setupEventListeners() ---
    
 // Vibe Chip Click Listener (FIXED: Clears Sidebar Conflicts)
    // Vibe Chip Click Listener (MULTI-SELECT VERSION)
    const vibeTrack = document.getElementById('vibe-track');
    if (vibeTrack) {
        vibeTrack.addEventListener('click', (e) => {
            const btn = e.target.closest('.vibe-chip');
            if (!btn) return;

            const filter = btn.dataset.filter;
            
            // 1. Multi-Toggle Logic
            if (currentTagFilters.includes(filter)) {
                // If it's already selected, remove it (Deselect)
                currentTagFilters = currentTagFilters.filter(t => t !== filter);
            } else {
                // If not selected, add it (Select)
                currentTagFilters.push(filter);
            }

            // 2. Sidebar Logic: If ANY chip is active, clear sidebar to prevent sticky conflicts
            if (currentTagFilters.length > 0) {
                document.querySelectorAll('.facet-checkbox').forEach(cb => cb.checked = false);
                
                // Reset Price Slider to "All" if necessary
                const slider = document.getElementById('price-slider');
                if(slider) {
                    slider.value = slider.max; 
                    currentPriceMax = Infinity;
                    const priceDisplay = document.getElementById('price-display-val');
                    if(priceDisplay) priceDisplay.textContent = "All";
                }
            }
            ...
            updateProductView();
            renderVibeScroller(); // Re-render to show multiple highlights
        });
    }
...
function displayMenu(inputData) {
    const navLinksContainer = document.getElementById('nav-links');
    const mobileNavLinks = document.getElementById('mobile-nav-links');
    if (!navLinksContainer || !mobileNavLinks) return;

    // --- 1. TARGET THE 'ITEMS' ARRAY ---
    let safeData = [];
    if (inputData && Array.isArray(inputData.items)) {
        safeData = inputData.items;
    } else if (Array.isArray(inputData)) {
        safeData = inputData;
    }

    if (!safeData || safeData.length === 0) {
        console.warn("displayMenu: No menu items found.");
        return;
    }
        ...
        if (item.isMegaMenu && Array.isArray(item.subMenu) && item.subMenu.length > 0) {
            ...
            desktopMenuHtml += `
                <div class="nav-item-with-mega-menu">
                    <span class="nav-links-desktop-item ${saleClass}" ${dataAttrs}>${title}</span>
                    <div class="mega-menu-panel"><div class="mega-menu-content">${subMenuLinks}</div></div>
                </div>`;
            ...
            mobileMenuHtml += `
                <div class="mobile-nav-accordion">
                    ...
                    <div class="mobile-nav-accordion-content">${subMenuLinks}</div>
                </div>`;
        } else {
            // Standard Link - Added dataAttrs here too
            desktopMenuHtml += `<a href="${linkTarget}" class="nav-links-desktop-item ${saleClass}" ${dataAttrs}>${title}</a>`;
            mobileMenuHtml += `<a href="${linkTarget}" class="mobile-nav-link-item ${saleClass}" ${dataAttrs}>${title}</a>`;
        }
    });
    ...
    document.querySelectorAll('.mobile-nav-accordion-header').forEach(btn => {
...
async function fetchWithAuth(url, options = {}) {
    ...
    try {
        ...
        const response = await fetch(`/api/vouchers?code=${trimmedCode}`);
    ...
    if (auth.isLoggedIn()) {
        try {
            ...
            const results = await Promise.allSettled([
                fetchWithAuth('/api/user-profile?type=address'),
                // THIS LINE IS THE FIX: It forces the browser to ignore its cache for orders.
                fetchWithAuth('/api/orders', { cache: 'no-cache' }), 
...
async function fetchProducts() {
    ...
    if (productGrid) {
        ...
        console.error("🔴 RED DOT ERROR: #product-grid element not found in DOM!");
    }

    const productsData = await fetchData('/api/inventory?type=product'); 
    
    // DIAGNOSTIC LOG
    console.log("🔴 RED DOT: Raw Product Data received:", productsData);

    if (productsData && !productsData.error) {
        // Handle case where API returns object wrapper { products: [...] }
        const items = Array.isArray(productsData) ? productsData : (productsData.products || []);
        
        console.log(`🔴 RED DOT: Processing ${items.length} products...`);
        ...
        allProducts = items.map(product => {
            ...
            if (typeof parsedPrice !== 'number' || isNaN(parsedPrice)) {
                console.warn(`[FETCH ERROR] Product ID ${product.id} has invalid price. Setting to 0.`);
                parsedPrice = 0;
            }
        ...
        console.log("🔴 RED DOT: allProducts array populated with:", allProducts.length, "items.");
        
        // 2. Once data is fetched, call updateProductView
        updateProductView(); 
    } else {
        console.error("Failed to fetch products:", productsData ? productsData.error : "No data returned");
        allProducts = [];
        currentlyDisplayedProducts = [];
        if (productGrid) {
            productGrid.innerHTML = `<p style="color: red; text-align: center;">Error: Could not load products. Please refresh.</p>`;
        }
    }
}
...
// REPLACE your existing fetchMenu function in public/app.js
async function fetchMenu() {
    console.log("fetchMenu: Starting...");
    try {
        // 1. FORCE the correct URL parameter: type=menu
        // This ensures site-settings.js looks at 'siteContent/header_nav' (your DB data)
        const url = '/api/site-settings?type=menu'; 
        console.log("fetchMenu: Fetching from:", url);
        ...
        const data = await response.json();
        console.log("fetchMenu: Raw Data Received:", data);

        // 2. Pass data to display function
        if (data) {
            displayMenu(data);
        } else {
            console.error("fetchMenu: Data is empty/null.");
        }
    } catch (error) {
        console.error("fetchMenu: Network Error:", error);
    }
}
...
async function fetchFooterInfo() {
    ...
    // FIX: Call the API endpoint instead of looking for a static .json file
...
async function fetchCustomHamperItems() {
    ...
    const customHamperData = await fetchData('/api/inventory?type=component');
...
async function fetchTestimonials() {
    console.log("fetchTestimonials: Fetching from API...");
    // FIX: Point to the API instead of 'data/testimonials.json'
    const data = await fetchData('/api/content-manager?page=testimonials');
    
    if (data && Array.isArray(data.testimonials)) {
        displayTestimonials(data.testimonials);
    } else {
        console.warn("No testimonials found in API.");
    }
}
...
async function updateProductView() {
    console.log("🔴 RED DOT: updateProductView STARTED.");
    ...
    // --- HELPER: SMART MATCH (Checks 1 Tag against 1 Product) ---
    const checkSmartMatch = (p, tag) => {
        const tagLower = tag.toLowerCase().trim();
        
        // A. Virtual Tag/Keyword Match (Deep Search "The Brain")
        const virtualKey = typeof VIRTUAL_TAGS !== 'undefined'
            ? Object.keys(VIRTUAL_TAGS).find(k => k.toLowerCase() === tagLower)
            : null;
        
        if (virtualKey) {
            const keywords = VIRTUAL_TAGS[virtualKey]
                .map(k => k.toLowerCase().trim())
                .filter(k => k.length > 0);
            keywords.push(tagLower); // Self-search fix

            // Build the giant text blob
            const bigString = [
                p.title, 
                p.description, 
                p.professionalDescription, 
                (Array.isArray(p.bullets) ? p.bullets.join(' ') : p.bullets), 
                (Array.isArray(p.contents) ? p.contents.join(' ') : p.contents),
                p.tag
            ].map(txt => String(txt || '').toLowerCase()).join(' ');

            if (keywords.some(k => k && bigString.includes(k))) return true;
        }
        
        // B. Strict/Manual Match (Fallback)
        const allTags = [
            ...(p.dietaryTags || []), 
            ...(p.occasionTags || []), 
            ...(p.contentsTags || []),
        ...
        if (allTags.includes(tagLower)) return true;
        if (tagLower === 'bestseller' && (p.tag?.toLowerCase().includes('best') || p.tag?.toLowerCase().includes('popular'))) return true;
        if (tagLower === 'sale' && p.tag?.toLowerCase().includes('sale')) return true;

        return false;
    };
    
    // --- INPUT GATHERING ---
    let selectedDietary = [], selectedOccasion = [], selectedContents = [];
    
    // LOGIC: If ANY chips are active, IGNORE sidebar inputs.
    if (currentTagFilters.length === 0) {
        selectedDietary = Array.from(document.querySelectorAll('input[data-type="dietary"]:checked')).map(cb => cb.value);
        selectedOccasion = Array.from(document.querySelectorAll('input[data-type="occasion"]:checked')).map(cb => cb.value);
        selectedContents = Array.from(document.querySelectorAll('input[data-type="contents"]:checked')).map(cb => cb.value);
    } else {
        document.querySelectorAll('.facet-checkbox').forEach(cb => cb.checked = false);
    }
    ...
    // --- STEP 1: BASE LIST ---
    if (searchTerm) {
        const options = { keys: ['title', 'category', 'professionalDescription', 'contents'], threshold: 0.3 };
        const fuse = new Fuse(allProducts, options);
        const results = fuse.search(searchTerm);
        productsToDisplay = results.map(result => result.item);
        currentTitle = `Results for "${searchTerm}"`;
    } else {
        productsToDisplay = [...allProducts];
        // Category Filter
        if (currentCategoryFilter && currentCategoryFilter !== 'all' && !currentCategoryFilter.startsWith('price-')) {
            const simpleFilter = currentCategoryFilter.replace(/s$/, '').toLowerCase();
            currentTitle = currentCategoryFilter.charAt(0).toUpperCase() + currentCategoryFilter.slice(1);
    ...
    // 1. Sidebar Facets (Only if no chips)
    if (selectedDietary.length > 0) productsToDisplay = productsToDisplay.filter(p => p.dietaryTags && selectedDietary.some(tag => p.dietaryTags.includes(tag)));
    if (selectedOccasion.length > 0) productsToDisplay = productsToDisplay.filter(p => p.occasionTags && selectedOccasion.some(tag => p.occasionTags.includes(tag)));
    if (selectedContents.length > 0) productsToDisplay = productsToDisplay.filter(p => p.contentsTags && selectedContents.some(tag => p.contentsTags.includes(tag)));

    // 2. Multi-Chip Filter (AND Logic)
    if (currentTagFilters.length > 0) {
        productsToDisplay = productsToDisplay.filter(p => {
            // The product must match EVERY selected chip
            return currentTagFilters.every(filterTag => checkSmartMatch(p, filterTag));
        });
    }

    // 3. Price & Sorting (No changes)
    ...
    } else if (currentPriceMax !== Infinity) {
        productsToDisplay = productsToDisplay.filter(p => p.price <= currentPriceMax);
    }
    ...
    if (productsSectionTitle) productsSectionTitle.textContent = currentTitle;
...
function displayProducts(products, gridElement = document.getElementById('product-grid')) {
    // 1. SELF-HEALING: If the grid is missing, create it automatically.
    if (!gridElement) {
        let container = document.getElementById('products-section') || document.getElementById('page-list');
        if (!container) return; // Exit if no container found
        
        gridElement = document.createElement('div');
        gridElement.id = 'product-grid';
        gridElement.className = 'product-grid';
    ...
    const productsSection = document.getElementById('products-section');
    const isSearchActive = productsSection ? productsSection.classList.contains('search-active') : false;
    ...
    gridElement.innerHTML = products.map((product, index) => {
        ...
        const styleAttr = `style="animation-delay: ${delay}s"`;
        ...
        const isPlaceholder = primaryImageUrl && primaryImageUrl.includes('placehold.co');
        const containerClass = isPlaceholder ? 'product-image-container luxury-card' : 'product-image-container';
        ...
        let stockInfoHtml = isOutOfStock 
            ? '<p class="out-of-stock-message">Currently Unavailable</p>' 
            : (isLowStock ? `<div class="badge-limited-edition"><i class="fa-solid fa-bolt-lightning"></i> Limited: Only ${product.stock} remain</div>` : '<div class="stock-placeholder"></div>');

        const dotsHtml = hasCarousel ? `<div class="carousel-dots">${imageUrls.map((_, i) => `<div class="dot ${i === 0 ? 'active' : ''}" data-index="${i}"></div>`).join('')}</div>` : '';
        const carouselArrowsHtml = hasCarousel ? `<button class="carousel-arrow prev" data-direction="-1"><i class="fa-solid fa-chevron-left"></i></button><button class="carousel-arrow next" data-direction="1"><i class="fa-solid fa-chevron-right"></i></button>` : '';
        const starRatingHtml = product.rating ? `<div class="star-rating">${generateStarRating(product.rating)}<span class="review-count">(${product.reviewCount || 0})</span></div>` : '<div class="star-rating-placeholder"></div>';
        const priceHtml = (product.salePrice && product.salePrice < product.price)
            ? `<div class="price-container sale"><span class="sale-price">${currency}${product.salePrice.toFixed(2)}</span><span class="original-price">${currency}${product.price.toFixed(2)}</span></div>`
            : `<p class="product-price">${currency}${product.price.toFixed(2)}</p>`;

        const imageHtml = `
            <div class="${containerClass}" data-product-id="${product.id}" data-images="${imageUrls.join(',')}" data-current-index="0">
                <img src="${primaryImageUrl}" alt="${product.title}" class="product-image" loading="lazy">
                ${enableQuickView ? `<button class="quick-view-btn" data-product-id="${product.id}">Quick View</button>` : ''} 
                <button class="wishlist-btn ${isWishlisted ? 'favorited' : ''}" data-product-id="${product.id}"><i class="fa-solid fa-heart"></i></button>
                ${product.tag ? `<div class="${tagClass}">${product.tag}</div>` : ''}
                ${carouselArrowsHtml}${dotsHtml}
            </div>
        `;

        const btnActionText = isTouchDevice ? 'Hold to Add' : 'Add to Basket';
        const addToBasketBtnHtml = `<button class="add-to-basket-btn" data-product-id="${product.id}" ${isOutOfStock ? 'disabled' : ''}><div class="hold-progress-bar"></div><span class="btn-label">${isOutOfStock ? 'Out of Stock' : btnActionText}</span></button>`;
...
// --- REPLACE YOUR EXISTING showProductDetail FUNCTION WITH THIS ---

// --- MASTER PRODUCT DETAIL FUNCTION ---
async function showProductDetail(slug) {
    const product = allProducts.find(p => createSlug(p.slug) === createSlug(slug));
    if (!product) return showPage('list');

    showPage('detail');
    
    // 1. Reset Container & Force Layout
    const container = document.getElementById('page-detail');
    container.innerHTML = '';
    // Ensure full width breakout
    ...
    if (hasVariants) {
        ...
        const grouped = product.variants.reduce((acc, v) => {
            const cleanType = capitalize(v.type);
            if (!acc[cleanType]) acc[cleanType] = [];
            acc[cleanType].push(v);
            return acc;
        }, {});

        Object.keys(grouped).forEach(type => {
            variantsHtml += `
                <div class="variant-group text-center md:text-left">
                    <label class="variant-label" for="select-${type}">${type}</label>
                    <div class="select-wrapper inline-block md:block">
                        <select class="variant-select" id="select-${type}" data-type="${type}">
                            ${grouped[type].map((v, index) => {
                                const isSelected = index === 0 ? 'selected' : ''; 
                                return `<option value="${v.value}" data-price-mod="${v.priceMod || 0}" ${isSelected}>${v.value}</option>`;
                            }).join('')}
                        </select>
    ...
    const priceDisplayHtml = hasSale 
        ? `<div class="price-container sale justify-center md:justify-start"><span class="sale-price" id="detail-price-display">£${currentPriceBase.toFixed(2)}</span><span class="original-price text-gray-400 line-through ml-2">£${product.price.toFixed(2)}</span></div>`
        : `<div class="price-container mb-4 flex justify-center md:justify-start"><span class="text-3xl font-medium" id="detail-price-display">£${product.price.toFixed(2)}</span></div>`;

    container.innerHTML = `
        <div class="page-header container mx-auto px-4 py-4">
            <button class="btn btn-secondary" onclick="showPage('list')">&larr; Back to Collection</button>
        </div>

        <div class="detail-grid relative">
            
            <div id="image-zoom-result" class="img-zoom-result"></div>

            <div class="gallery-wrapper relative">
                <div class="detail-image-container" id="detail-gallery-track">
                    ${imageUrls.map((url, index) => `
                       <div class="gallery-slide">
    <img id="product-image-${index}" src="${url}" class="detail-carousel-image zoom-target" alt="${product.title}" data-index="${index}">
</div>
                    `).join('')}
                </div>
                ${imageUrls.length > 1 ? `
        ...
        <div class="review-wall-section mt-16 bg-gray-50 py-12">
    ...
    // 5. Initialize Logic
    initDetailLogic(product, currentPriceBase, hasVariants);

    // 6. EXTRAS & HELPERS
    if (typeof loadCrossSells === 'function') loadCrossSells(product);
    if (typeof loadReviewsForWall === 'function') loadReviewsForWall(product.id);
    
    // START GALLERY
    initGalleryScroll(imageUrls.length);

    // START RELATED PRODUCTS
    renderRelatedProducts(product); 

    // START ZOOM (Delayed to ensure DOM is ready)
    setTimeout(() => {
        document.querySelectorAll('.zoom-target').forEach(img => initImageZoom(img));
    }, 500);
}
...
function initImageZoom(img) {
    ...
    // 2. Cleanup & Create Global Zoom Window
    let result = document.getElementById('my-global-zoom');
    if (!result) {
        result = document.createElement('div');
        result.id = 'my-global-zoom';
    ...
    lens.style.border = "1px solid #888";
    ...
    lens.style.pointerEvents = "none"; // Important for smooth movement
    ...
    const startZoom = () => {
        ...
        // Calculate Ratios
        const resW = result.offsetWidth || 500;
        const resH = result.offsetHeight || 500;
        const lensW = lens.offsetWidth || 100;
        const lensH = lens.offsetHeight || 100;
    ...
    const stopZoom = () => {
        result.style.display = "none";
        lens.style.display = "none";
        img.parentElement.removeEventListener("mousemove", moveLens);
    };
    ...
    // Attach listeners to the PARENT container for stability
    img.parentElement.addEventListener("mouseenter", startZoom);
    img.parentElement.addEventListener("mouseleave", stopZoom);
}
...
function renderRelatedProducts(currentProduct) {
    ...
    placeholder.innerHTML = `
        <div class="related-products-section container mx-auto px-4 mt-16 border-t pt-8">
            <h3 class="section-title text-center mb-8 font-headings text-2xl">You Might Also Like</h3>
            <div class="related-products-grid" style="display:grid; grid-template-columns: repeat(auto-fill, minmax(240px, 1fr)); gap:20px;">
                ${cardsHtml}
            </div>
        </div>
    `;
...
// 4. BUTTONS & LOGIC HELPER
function initDetailLogic(product, basePrice, hasVariants) {
    ...
    function updatePrice() {
        ...
        document.querySelectorAll('.variant-select').forEach(select => {
            ...
            totalMod += parseFloat(selectedOption.dataset.priceMod || 0);
        });
        currentPrice = basePrice + totalMod;
        const qty = parseInt(qtyInput.value) || 1;
        
        if (priceDisplay) priceDisplay.innerText = `£${currentPrice.toFixed(2)}`;
        addBtn.innerText = `Add to Basket - £${(currentPrice * qty).toFixed(2)}`;
    }

    document.getElementById('qty-plus').addEventListener('click', () => { 
        qtyInput.value = parseInt(qtyInput.value) + 1; updatePrice(); 
    });
    document.getElementById('qty-minus').addEventListener('click', () => { 
        if (qtyInput.value > 1) qtyInput.value = parseInt(qtyInput.value) - 1; updatePrice(); 
    });
    ...
    addBtn.addEventListener('click', () => {
        ...
        if (hasVariants) {
            const selections = [];
            document.querySelectorAll('.variant-select').forEach(select => selections.push(`${select.dataset.type}: ${select.value}`));
            variantDesc = selections.join(', ');
        }
        const qty = parseInt(qtyInput.value) || 1;
        
        if (typeof addToCart === 'function') {
            addToCart(product.id, qty, false, [], currentPrice, variantDesc);
    ...
    if (wishlistBtn && typeof wishlist !== 'undefined') {
        if (wishlist.isWishlisted(product.id)) {
            wishlistBtn.innerHTML = '<i class="fa-solid fa-heart text-red-500 text-xl"></i>';
        ...
        wishlistBtn.addEventListener('click', () => {
            ...
            wishlistBtn.innerHTML = isWish ? '<i class="fa-solid fa-heart text-red-500 text-xl"></i>' : '<i class="fa-regular fa-heart text-xl"></i>';
            wishlistBtn.classList.toggle('border-red-500', isWish);
        });
    }
}

// --- HELPER: Fetch & Render Components for Cross-Sell ---
// --- HELPER: Fetch & Render Components for Cross-Sell ---
async function loadCrossSells(product) {
    try {
        ...
        // Simple logic: Pick 3 random components
            ...
            grid.innerHTML = suggestions.map(c => `
                <div class="mini-product-card">
                    ...
                    <button class="mini-add-btn" 
                        ...
                        data-title="${c.name}" 
                        data-price="${c.price}" 
                        data-image="${c.imageUrl1}">
                        + Add
                    </button>
                </div>
            `).join('');

            // 2. Attach Event Listeners Properly
            grid.querySelectorAll('.mini-add-btn').forEach(btn => {
                btn.addEventListener('click', (e) => {
                    ...
                    // Use the globally available cart instance (imported in app.js)
                    cart.addItem(item); 
                    
                    // Visual Feedback
                    const originalText = btn.innerText;
                    btn.innerText = "✓";
                    btn.style.backgroundColor = "#4ade80"; // Light green
                    setTimeout(() => {
...
// --- HELPER: Load Reviews for Bottom Wall ---
async function loadReviewsForWall(productId) {
    const grid = document.getElementById('review-wall-grid');
    try {
        // Re-use your testimonials API or reviews API
        const reviews = await fetchData(`/api/reviews?productId=${productId}`); // Assumes you have this
        // Fallback if no specific product reviews: Show generic Testimonials
        if (!reviews || reviews.length === 0) {
             const generic = await fetchData('/api/content-manager?page=testimonials');
             if(generic && generic.testimonials) {
                 renderReviewsToGrid(generic.testimonials, grid);
             } else {
                 grid.innerHTML = '<p class="text-center col-span-3">No reviews yet.</p>';
             }
        } else {
            renderReviewsToGrid(reviews, grid);
        }
    } catch (e) {
        grid.innerHTML = '';
    }
...
function showAllProducts() {
    // --- FIX: MEMORY MODE ---
    // If coming back from a product, just show the list as we left it.
    if (returnToSearchState) {
        console.log("showAllProducts: Returning to list, preserving search state.");
        returnToSearchState = false; // Reset the flag for next time
        showPage('list');
        return; // STOP here! Do not wipe the search.
    }
    // ------------------------
...
function showPage(pageId) {
    ...
    // --- THE FIX IS HERE ---
    // We added 'pageReviewForm' to this list so it gets hidden properly
    const pages = [
        ...
        pageReviewForm // <--- THIS WAS MISSING
    ];
    ...
    if (targetPage) {
        targetPage.style.display = (pageId === 'list') ? 'flex' : 'block';
        window.scrollTo({ top: 0, behavior: 'smooth' });
    }
}
...
function showConfirmationModal(message, callback) {
    console.log("🔴 RED DOT: Opening Modal for:", message);

    const confirmBtn = document.getElementById('modal-confirm-btn');
    const overlay = document.getElementById('confirmation-modal-overlay');
    const msgEl = document.getElementById('modal-message');
    const reviewForm = document.getElementById('page-review-form');

    if (!overlay || !confirmBtn || !msgEl) {
        console.error("🔴 RED DOT: Modal elements missing! Fallback to alert.");
        alert(message); 
    ...
    // Hide the review form page if it's visible to prevent overlay issues
    if (reviewForm && reviewForm.style.display !== 'none') {
        reviewForm.style.display = 'none';
    }
    
    // ACTION: Force the modal to the very top layer of the browser
    overlay.style.setProperty('display', 'flex', 'important');
    overlay.style.setProperty('position', 'fixed', 'important');
    overlay.style.setProperty('z-index', '2147483647', 'important'); // Max 32-bit int
    ...
    // Action: Ensure the actual modal box is also visible
    const modalBox = document.getElementById('confirmation-modal');
    if (modalBox) {
    ...
    // ACTION: Remove old event listeners and attach fresh one
    confirmBtn.onclick = null;
    confirmBtn.removeEventListener('click', arguments.callee);
    
    confirmBtn.textContent = callback ? 'Confirm' : 'OK';
    confirmBtn.onclick = (e) => {
        e.preventDefault();
        e.stopPropagation();
        console.log("🔴 RED DOT: User clicked OK. Executing callback...");
        hideConfirmationModal();
        if (typeof callback === 'function') callback();
    };
}
function hideConfirmationModal() {
    console.log("🔴 RED DOT: hideConfirmationModal called.");
    if (confirmationModalOverlay) {
        confirmationModalOverlay.style.setProperty('display', 'none', 'important');
        confirmationModalOverlay.style.setProperty('opacity', '0', 'important');
        confirmationModalOverlay.style.setProperty('visibility', 'hidden', 'important');
    }
    if (confirmationModal) {
        confirmationModal.style.setProperty('display', 'none', 'important');
    }
...
async function fetchAndDisplayStaticPage(pageName) {
    ...
    let pageContent;
    let endpoint = ''; // Variable to hold API endpoint or JSON path
    ...
    switch(pageName) {
        case 'about_us':
            ...
            break;
            case 'privacy_policy':
            endpoint = '/api/content-manager?page=privacy_policy';
            break;
            case 'terms_and_conditions':
            endpoint = '/api/content-manager?page=terms_and_conditions';
            break;
        // Add cases for other dynamic pages here later (e.g., contact_us)
         case 'contact_us':
            ...
            break;
        default:
            endpoint = `data/pages/${pageName}.json`; // Default to JSON file
            break;
    }

    // Fetch the content
    try {
        pageContent = await fetchData(endpoint); // Assumes fetchData handles JSON parsing
        console.log(`[fetchAndDisplayStaticPage] Raw content received for '${pageName}':`, pageContent);
        if (!pageContent) throw new Error(`No content received from ${endpoint}`);

    } catch (error) {
         console.error(`Error fetching content for ${pageName} from ${endpoint}:`, error);
        pageContent = null; // Set to null on error
    }
    ...
    if (pageContent) {
        ...
        } else if (pageName === 'about_us' && pageContent.pageTitle && Array.isArray(pageContent.sections)) { // Specific check for About Us structure
             renderGenericStaticPage(pageContent, pageName, pageContent.pageTitle); // Pass title override
         } else if (pageName === 'our_mission' && pageContent.pageTitle && Array.isArray(pageContent.sections)) {
             renderGenericStaticPage(pageContent, pageName, pageContent.pageTitle);  
             } else if (pageName === 'privacy_policy' && pageContent.pageTitle && Array.isArray(pageContent.sections)) {
             renderGenericStaticPage(pageContent, pageName, pageContent.pageTitle);  
             } else if (pageName === 'terms_and_conditions' && pageContent.pageTitle && Array.isArray(pageContent.sections)) {
             renderGenericStaticPage(pageContent, pageName, pageContent.pageTitle);
        } else if (pageContent.sections && Array.isArray(pageContent.sections)) { // Generic fallback assumes 'sections' array
             renderGenericStaticPage(pageContent, pageName, pageContent.sections[0]?.title);
        } else {
            console.error(`Content structure for '${pageName}' from ${endpoint} is unexpected.`);
        ...
        // Render page not found if fetch failed or content was null
...
// --- REPLACE EXISTING addToCart FUNCTION IN public/app.js ---

function addToCart(productId, quantity = 1, isCustom = false, customItems = [], overridePrice = 0, variantDetails = null) {
    if (isCustom) {
        // Custom Hamper Logic
        ...
        const customHamper = {
            ...
            price: overridePrice, // Uses the passed total
            quantity: 1,
        ...
        // Determine final price: Use override if provided (for variants), otherwise base price
        const finalPrice = (variantDetails && overridePrice > 0) ? overridePrice : product.price;

        // Check if item exists with SAME ID *AND* SAME VARIANTS
        ...
        if (existingItem) {
            ...
            cart.push({ 
                ...product, 
                quantity: quantity, 
                price: finalPrice, // <--- Save the updated price!
                variantDetails: variantDetails, // <--- Save the "Size: Large" string
                isCustom: false 
            });
        }
    ...
    if (window.innerWidth < 768) {
        const addedItem = isCustom ? cart[cart.length - 1] : cart.find(item => item.id.toString() === productId.toString() && item.variantDetails === variantDetails);
...
const handleAddressSearch = debounce(async () => {
    ...
    try {
        const response = await fetch(`/api/address-proxy?term=${encodeURIComponent(term)}`);
...
async function placeOrder() {
    ...
    try {
        ...
        // We check local stock data before even hitting the API
...
async function selectAddressSuggestion(id) {
    ...
    try {
        const response = await fetch(`/api/address-proxy?id=${id}`);
...
async function handleOrderCancellation(orderId, type) {
    ...
    showConfirmationModal(confirmationMessage, async () => {
        try {
            await fetchWithAuth(`/api/admin-orders?action=cancel`, {
                method: 'POST',
                body: JSON.stringify({
                    orderId,
...
async function renderMyOrdersPage(skipRefresh = false) {
    console.log("[renderMyOrdersPage] Starting...");

    // 1. Wait for initial auth only if we haven't loaded data yet
    if (!initialUserDataPromise && userOrders.length === 0) {
    ...
    if (!skipRefresh) {
        try {
            ...
            userOrders = await fetchWithAuth('/api/orders', { cache: 'no-cache' });
            console.log("[renderMyOrdersPage] Refreshed.");
        } catch (error) {
            console.error("[renderMyOrdersPage] Refresh failed, using local data:", error.message);
        }
    } else {
        console.log("[renderMyOrdersPage] Skipping network refresh for instant transition.");
    }

    // 3. Render the Page (Same as before)
    const controlsHtml = `
    ...
    document.getElementById('orders-back-to-account')?.addEventListener('click', () => router.navigate('/account'));
}
...
async function renderReviewFormPage(orderId) {
    console.log("🔴 RED DOT: Rendering Visible Stars for", orderId);

    if (window.initialUserDataPromise) {
        await window.initialUserDataPromise;
    }

    const order = userOrders.find(o => String(o.id) === String(orderId));

    if (!order) {
        showConfirmationModal("Order not found. Reloading...", () => location.reload());
        return;
    }

    const reviewItemsHtml = order.items.map(item => {
        const product = allProducts.find(p => String(p.id) === String(item.productId));
        const img = (product && product.imageUrls) ? product.imageUrls[0] : 'https://placehold.co/50x50?text=Item';
        ...
        <div class="review-item-card" data-product-id="${item.productId}" style="border-bottom:1px solid #eee; padding:20px 0;">
            ...
            <div class="star-rating-wrapper" style="margin-bottom: 15px;">
                <div class="star-rating-input">
                    <input type="radio" id="s-${item.productId}-5" name="rating-${item.productId}" value="5">
                    <label for="s-${item.productId}-5" title="5 Stars">★</label>
                    
                    <input type="radio" id="s-${item.productId}-4" name="rating-${item.productId}" value="4">
                    <label for="s-${item.productId}-4" title="4 Stars">★</label>
                    
                    <input type="radio" id="s-${item.productId}-3" name="rating-${item.productId}" value="3">
                    <label for="s-${item.productId}-3" title="3 Stars">★</label>
                    
                    <input type="radio" id="s-${item.productId}-2" name="rating-${item.productId}" value="2">
                    <label for="s-${item.productId}-2" title="2 Stars">★</label>
                    
                    <input type="radio" id="s-${item.productId}-1" name="rating-${item.productId}" value="1">
                    <label for="s-${item.productId}-1" title="1 Star">★</label>
                </div>
            </div>

            <textarea id="review-text-${item.productId}" placeholder="Write your review here..." style="width:100%; padding:10px; border:1px solid #ccc; border-radius:4px; min-height:80px;"></textarea>
        </div>`;
    }).join('');
    ...
    pageReviewForm.innerHTML = `
        <style>
            ...
            .star-rating-input {
                display: flex;
                flex-direction: row-reverse; /* Important for hover effect */
            ...
            .star-rating-input label {
                font-size: 35px; /* Make them BIG */
                color: #ccc; /* Grey by default */
                cursor: pointer;
                line-height: 1;
                transition: color 0.2s;
            }
            /* Color the clicked star and all before it (visually after due to row-reverse) */
            ...
            .star-rating-input label:hover ~ label {
                color: #ffdb70; /* Light Gold */
            }
        </style>

        <div class="page-header">
            <h2>Review Order #${order.id}</h2>
            <button class="btn btn-secondary" id="back-to-orders-review">Back</button>
        </div>
        <div style="max-width: 800px; margin: 0 auto; padding: 20px;">
            <form id="review-form">
                ${reviewItemsHtml}
                <button type="submit" id="submit-review-btn" class="btn btn-primary btn-full-width" style="margin-top:20px; padding: 15px; font-size: 1.1em;">
                    Submit Reviews
                </button>
            </form>
        </div>
    ...
    // Attach Listeners
    document.getElementById('back-to-orders-review').addEventListener('click', () => router.navigate('/account/orders'));
    document.getElementById('review-form').onsubmit = (e) => handleReviewSubmit(e, orderId);
}
...
async function handleReviewSubmit(e, orderId) {
    e.preventDefault();
    console.log("🔴 RED DOT: handleReviewSubmit triggered for:", orderId);

    const submitBtn = document.getElementById('submit-review-btn');
    if (submitBtn) submitBtn.disabled = true;

    // FIX: Properly initialize the reviews array before the loop
    const reviews = []; 
    ...
    reviewCards.forEach(card => {
        ...
        // Use the card scope to find specific rating and text inputs
        const ratingInput = card.querySelector(`input[name="rating-${productId}"]:checked`);
        const reviewText = card.querySelector(`#review-text-${productId}`).value;
    ...
    try {
        // Send the collected 'reviews' array to the server
        await fetchWithAuth('/api/reviews', {
            method: 'POST',
            body: JSON.stringify({ orderId: orderId, reviews: reviews })
        });

        // Update local order state so the change is visible immediately
        const order = userOrders.find(o => String(o.id) === String(orderId));
        if (order) order.status = 'Completed (Reviewed)';

        // TRIGGER SUCCESS MODAL: Navigation only happens inside the callback
        showConfirmationModal("Success! Your review has been saved.", () => {
            console.log("🔴 RED DOT: Navigating back to My Orders.");
            router.navigate('/account/orders'); 
        });

    } catch (error) {
        console.error("🔴 RED DOT: Submit failed:", error);
        showConfirmationModal(error.message); // Modal handles the error message
        if (submitBtn) submitBtn.disabled = false;
    }
}
// Keep the helper function to ensure visibility
...
function finalizeSubmission(orderId) {
    ...
    showConfirmationModal("Success! Reviews submitted (Simulation).", () => {
...
// Helper: Ignores case/hyphens so "alcohol-free" matches "Alcohol Free" or "alcohol"
function looseMatch(str1, str2) {
    ...
    // Check strict equality OR if one contains the other (for "Alcohol" matching "Contains Alcohol")
    return clean1 === clean2 || clean1.includes(clean2) || clean2.includes(clean1);
}
function renderVibeScroller() {
    ...
    const priceVal = document.getElementById('filter-select')?.value;
    
    // Determine active tags (Handling the Multi-Select Array we just added)
    const activeTags = (typeof currentTagFilters !== 'undefined' && Array.isArray(currentTagFilters)) 
        ? currentTagFilters 
    ...
    // Priority 2: Search Term
    else if (term.length > 2) {
        const foundContext = Object.keys(DISCOVERY_CONTEXTS).find(key => 
            DISCOVERY_CONTEXTS[key].triggers.some(trigger => term.includes(trigger))
        );
        if (foundContext) activeContext = foundContext;
    }
    ...
    activeTags.forEach(tag => {
        if (tag && !chipsToRender.includes(tag)) {
            chipsToRender.unshift(tag);
        }
    });

    // --- 4. RENDER UI ---
    const html = chipsToRender.map(filterVal => {
        // Check active state
        let isActive = false;
        if (typeof currentTagFilters !== 'undefined' && Array.isArray(currentTagFilters)) {
            isActive = currentTagFilters.includes(filterVal);
        } else if (typeof currentTagFilter !== 'undefined') {
            isActive = (currentTagFilter === filterVal);
        }
        ...
        if (filterVal === 'BESTSELLER') { iconClass = 'fa-solid fa-fire text-orange-500'; label = 'Trending'; }
        else if (filterVal.includes('price')) { iconClass = 'fa-solid fa-coins text-blue-500'; label = filterVal.replace('price-', 'Under £'); }
        else if (filterVal === 'Sale') iconClass = 'fa-solid fa-percent text-red-600';
        else if (lowerVal.includes('vegan')) iconClass = 'fa-solid fa-leaf text-green-500';
        else if (lowerVal.includes('gluten')) iconClass = 'fa-solid fa-wheat-awn-circle-exclamation text-yellow-600';
        else if (lowerVal.includes('alcohol free')) iconClass = 'fa-solid fa-ban text-red-500';
        else if (lowerVal.includes('cheese')) iconClass = 'fa-solid fa-cheese text-yellow-400';
        else if (lowerVal.includes('chocolate')) iconClass = 'fa-solid fa-cookie-bite text-brown-600';
        else if (lowerVal.includes('wine')) iconClass = 'fa-solid fa-wine-glass text-purple-600';
        else if (lowerVal.includes('birthday')) iconClass = 'fa-solid fa-cake-candles text-pink-500';
        else if (lowerVal.includes('baby')) iconClass = 'fa-solid fa-baby-carriage text-blue-300';
        else if (lowerVal.includes('thank')) iconClass = 'fa-solid fa-hand-holding-heart text-green-600';

        return `
            <button class="vibe-chip ${isActive ? 'active' : ''}" data-filter="${filterVal}">
...
// --- NEW FEATURE: PRICE HEATMAP ---
function renderPriceHeatmap() {
    const container = document.getElementById('price-heatmap-wrapper');
    if (!container) return; // Ensure you add <div id="price-heatmap-wrapper"></div> to your sidebar HTML

    // 1. Calculate Buckets (e.g., £0-20, £20-40...)
    const prices = allProducts.map(p => p.price);
    const maxPrice = Math.max(...prices, 100); // Default to at least 100 if empty
    const bucketCount = 10;
    const bucketSize = Math.ceil(maxPrice / bucketCount);
    ...
    prices.forEach(price => {
        const index = Math.min(Math.floor(price / bucketSize), bucketCount - 1);
        buckets[index]++;
    });

    const maxCount = Math.max(...buckets, 1); // Avoid divide by zero

    // 2. Generate HTML for Bars
    let barsHtml = '';
    buckets.forEach((count, index) => {
        const height = (count / maxCount) * 100; // Percentage height
        const rangeStart = index * bucketSize;
        barsHtml += `<div class="heatmap-bar active" style="height: ${height}%" data-index="${index}" title="${count} hampers around £${rangeStart}"></div>`;
    });
    ...
    container.innerHTML = `
        <div class="price-heatmap-container">
            <div class="heatmap-label">
                <span>Max Price:</span>
                <span id="price-display-val">All</span>
            </div>
            <div class="heatmap-bars" id="heatmap-bars">
                ${barsHtml}
            </div>
            <input type="range" id="price-slider" class="price-slider-input" min="0" max="${bucketCount}" value="${bucketCount}" step="1">
        </div>
    `;

    // 4. Attach Listeners
    const slider = document.getElementById('price-slider');
    const display = document.getElementById('price-display-val');
    const bars = document.querySelectorAll('.heatmap-bar');

    slider.addEventListener('input', (e) => {
        const val = parseInt(e.target.value);
        
        // Update Visuals (Active Bars)
        bars.forEach((bar, index) => {
            if (index < val) {
                ...
                bar.style.background = '#e5e7eb'; // Grey
            }
        });

        // Update Text & Trigger Filter
        const currentCap = val * bucketSize;
        if (val === bucketCount) {
            display.textContent = "All";
            currentPriceMax = Infinity; // Global variable we will create
        } else {
            display.textContent = `£${currentCap}`;
            currentPriceMax = currentCap;
        }
...
let holdTimer = null;
let holdProgress = 0;
const HOLD_DURATION = 800; // ms to complete the fill

function initHoldToBuy() {
    // 1. Mouse users get an immediate click
    document.addEventListener('click', (e) => {
        const btn = e.target.closest('.add-to-basket-btn');
        if (!btn || btn.disabled || isTouchDevice) return;
        
        e.preventDefault();
        e.stopPropagation(); // Stops the click from bubbling up to the product card link
        completePurchase(btn, btn.dataset.productId);
    });

    // 2. Touch users keep the premium hold gesture
    document.addEventListener('touchstart', startHold, { passive: false });
    document.addEventListener('touchend', cancelHold, true);
    document.addEventListener('mouseleave', cancelHold, true);
}
...
function startHold(e) {
    ...
    // Prevent scrolling or double-taps on mobile while holding
    if (e.type === 'touchstart') e.preventDefault();

    const progressBar = btn.querySelector('.hold-progress-bar');
    ...
    holdTimer = setInterval(() => {
        ...
        holdProgress = (elapsed / HOLD_DURATION) * 100;

        if (progressBar) progressBar.style.width = `${holdProgress}%`;

        if (holdProgress >= 100) {
            completePurchase(btn, productId);
        }
    }, 16);
}
...
function cancelHold() {
    if (holdTimer) {
        ...
        document.querySelectorAll('.hold-progress-bar').forEach(bar => {
            bar.style.width = '0%';
        });
    }
}

function completePurchase(btn, productId) {
    clearInterval(holdTimer);
    holdTimer = null;

    // 1. Physical Haptic (Mobile Only)
    ...
    // 3. Original Core Actions
    addToCart(productId, 1);
    triggerFlyToCartAnimation(productId);
}

// --- PHASE 4: SCARCITY FEED LOGIC ---
function initScarcityTicker() {
...
async function fetchDiscoveryConfig() {
    ...
    try {
        ...
        if (doc.exists) {
            ...
            return; // We have the data, we can stop here.
        } else {
...
// Helper to convert Database format (Array) -> App format (Object)
function applyDiscoveryData(data) {
    const newContexts = {};
    
    // Convert the Array from Admin Panel back to the Object Map the app expects
    ...
    // Always ensure the default exists
    if (!newContexts['default']) {
        newContexts['default'] = { 
            triggers: [], 
    ...
    DISCOVERY_CONTEXTS = newContexts;
    VIRTUAL_TAGS = data.virtual_tags || {};

    console.log(`🧠 Brain Loaded: ${Object.keys(DISCOVERY_CONTEXTS).length} contexts active.`);
    
    // Force a re-render of the chips immediately
    renderVibeScroller();
}

// --- HELPER: GALLERY SCROLL LOGIC ---
function initGalleryScroll(count) {
    const track = document.getElementById('detail-gallery-track');
    if (!track || count <= 1) return;
