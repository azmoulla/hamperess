...
function initCinematicScroll() {
    ...
    if (vibeTrack) {
        vibeTrack.addEventListener('click', (e) => {
            ...
            if (!chip) return; // Not a chip click

            e.preventDefault(); 
            const filterVal = chip.dataset.filter;
            
            // A. PRICE TOGGLE
            if (filterVal.startsWith('price-')) {
                const priceSelect = document.getElementById('filter-select');
                if (priceSelect) {
                    priceSelect.value = (priceSelect.value === filterVal) ? 'all' : filterVal;
                    updateProductView(); 
                }
                return;
            }

            // B. CHECKBOX TOGGLE (Tags)
            let matchingCheckbox = document.querySelector(`.facet-checkbox[value="${filterVal}"]`);
            if (!matchingCheckbox) {
                // Smart Match Fallback
                const allChecks = Array.from(document.querySelectorAll('.facet-checkbox'));
                matchingCheckbox = allChecks.find(cb => looseMatch(cb.value, filterVal));
            }
            ...
            // C. SPECIAL TAGS
            if (filterVal === 'BESTSELLER' || filterVal === 'SALE') {
                const targetTag = filterVal === 'BESTSELLER' ? 'BESTSELLER' : 'SALE';
                currentTagFilter = (currentTagFilter === targetTag) ? null : targetTag;
                updateProductView();
                return;
            }
            
            // We removed Categories from the chips, so we don't need Case D here anymore.
            // If you keep them, logic is same as before.
        });
    }
}
...
function displayMenu(menuItems) {
    console.log("--- DEBUG: displayMenu started ---"); // PROOF correct code is running
    ...
    // --- FIX: Unwrap the data correctly ---
    let safeData = [];
    
    // Check if we received the wrapper object { items: [...] }
    if (menuItems && Array.isArray(menuItems.items)) {
        console.log("DEBUG: Found .items wrapper. Unwrapping now.");
        safeData = menuItems.items;
    } 
    // Check if we received a plain array [...]
    else if (Array.isArray(menuItems)) {
        console.log("DEBUG: Received plain array.");
        safeData = menuItems;
    } 
    else {
        console.error("CRITICAL ERROR: Data is not an array and has no .items property:", menuItems);
...
async function fetchMenu() {
    try {
        ...
        // LOGGING: See exactly what the server sent
        console.log('Server Menu Response:', data);

        // Call our safe display function
        displayMenu(data);
...
async function updateProductView() {
    ...
    // 1. Collect Sidebar Filters
    const selectedDietary = Array.from(document.querySelectorAll('input[data-type="dietary"]:checked')).map(cb => cb.value);
    const selectedOccasion = Array.from(document.querySelectorAll('input[data-type="occasion"]:checked')).map(cb => cb.value);
    const selectedContents = Array.from(document.querySelectorAll('input[data-type="contents"]:checked')).map(cb => cb.value);

    let productsToDisplay = [];
    let currentTitle = "All Hampers";

    if (searchTerm) {
        const options = { keys: ['title', 'category', 'professionalDescription'], threshold: 0.3 };
        ...
        // --- TITLE LOGIC ---
        // 1. Set Title based on CATEGORY context only
        ...
        else if (currentTagFilter === 'BESTSELLER') currentTitle = "Trending Hampers";
        else if (currentTagFilter === 'SALE') currentTitle = "Special Offers";
        ...
        // 2. Sidebar Facets (The Tags)
        if (selectedDietary.length > 0) {
            productsToDisplay = productsToDisplay.filter(p => p.dietaryTags && selectedDietary.some(tag => p.dietaryTags.includes(tag)));
        }
        if (selectedOccasion.length > 0) {
            productsToDisplay = productsToDisplay.filter(p => p.occasionTags && selectedOccasion.some(tag => p.occasionTags.includes(tag)));
        }
        if (selectedContents.length > 0) {
            productsToDisplay = productsToDisplay.filter(p => p.contentsTags && selectedContents.some(tag => p.contentsTags.includes(tag)));
        }

        // 3. Special Tag Filter
        if (currentTagFilter) {
    ...
    // REPLACE 'syncChipVisuals()' WITH THIS:
    renderVibeScroller();
}
...
function displayProducts(products, gridElement = document.getElementById('product-grid')) {
    ...
    if (!gridElement) {
        console.warn("⚠️ Self-Healing: #product-grid missing. Attempting to create it...");
        
        // Try to find the standard container
        let container = document.getElementById('products-section');
        
        // If the container is also missing, try the main page wrapper
        if (!container) {
             container = document.getElementById('page-list');
             if (!container) {
                 console.error("❌ Critical: Neither #products-section nor #page-list found. Cannot render products.");
                 return;
             }
             // Create a container section if we had to fall back to page-list
             const section = document.createElement('section');
             section.id = 'products-section';
             section.innerHTML = '<div class="container"><h2 id="products-section-title" class="section-title">Our Collection</h2></div>';
             container.appendChild(section);
             // Update container to the new section
             container = section.querySelector('.container') || section; 
        } else {
             // If we found products-section, ensure we append to its inner container if possible
             const innerContainer = container.querySelector('.container');
             if (innerContainer) container = innerContainer;
        }

        // Create the missing grid
        gridElement = document.createElement('div');
        gridElement.id = 'product-grid';
        gridElement.className = 'product-grid';
        container.appendChild(gridElement);
        console.log("✅ Self-Healing: #product-grid created and attached to DOM.");
    }
    ...
    console.log(`🔴 RED DOT: Rendering ${products.length} products to the grid.`);
...
async function showProductDetail(slug) {
    ...
    // Fetch delivery info (existing logic)
    ...
    // Star rating HTML
    const starRatingHtml = product.rating 
        ? `<div class="star-rating detail-page-stars">${generateStarRating(product.rating)}<span class="review-count">(${product.reviewCount || 0} reviews)</span></div>` 
        : '<div class="star-rating detail-page-stars"><span class="review-count">No reviews yet</span></div>';

    // --- RENDER HTML (With New Reviews Tab) ---
    pageDetail.innerHTML = `
            ...
            <div class="detail-info">
                ...
                <div class="product-tabs-container">
                    <div class="tab-nav">
                        ...
                        <button class="tab-link" data-tab="reviews">Reviews</button> </div>
                    <div class="tab-content-container">
                        ...
                        <div class="tab-content" id="reviews-tab">
                            <div id="reviews-content-container"></div> </div>
                    </div>
                </div>
...
function renderVibeScroller() {
    ...
    // 1. Define the "Money Maker" Defaults
    // Use the exact spacing/spelling you want displayed on the button
    const defaults = [
        { label: 'Trending', value: 'BESTSELLER', icon: 'fa-solid fa-fire text-orange-500' },
        { label: 'Under £50', value: 'price-0-50', icon: 'fa-solid fa-tag text-blue-500' },
        { label: 'Vegan', value: 'Vegan', icon: 'fa-solid fa-leaf text-green-500' },
        { label: 'Alcohol Free', value: 'Alcohol Free', icon: 'fa-solid fa-ban text-red-500' },
        { label: 'Gluten Free', value: 'Gluten Free', icon: 'fa-solid fa-wheat-awn-circle-exclamation text-yellow-600' }
    ];

    // 2. Capture currently active filters from the Sidebar & Sort
    const activeFilters = [];
    const allCheckboxes = Array.from(document.querySelectorAll('.facet-checkbox'));
    
    // Check Sidebar Inputs (The Dynamic Part)
    allCheckboxes.forEach(cb => {
        if (cb.checked) {
            // Check if this checked item is already in our defaults
            const isDefault = defaults.some(d => looseMatch(d.value, cb.value));
            
            // Only add to "Active Stack" if it's NOT a default chip
            if (!isDefault) {
                activeFilters.push({ 
                    label: cb.value, 
                    value: cb.value, 
                    icon: 'fa-solid fa-check text-gold-500', 
                    isActive: true
                });
            }
        }
    ...
    // A. Render Dynamic Active Chips (First)
    activeFilters.forEach(tag => {
        html += `<button class="vibe-chip active" data-filter="${tag.value}">
                    <i class="${tag.icon}"></i> ${tag.label}
                 </button>`;
    });

    // B. Render Defaults (checking if they are active using SMART MATCH)
    const priceValue = document.getElementById('filter-select')?.value || 'all';
    
    defaults.forEach(def => {
        let isActive = false;

        // Check Price State
        if (def.value.startsWith('price-')) {
            isActive = (def.value === priceValue);
        }
        // Check Special Tag State
        else if (def.value === 'BESTSELLER') {
            isActive = (currentTagFilter === 'BESTSELLER');
        }
        // Check Sidebar State (THE FIX IS HERE)
        else {
            // Instead of strict querySelector, we find via looseMatch
            const checkbox = allCheckboxes.find(cb => looseMatch(cb.value, def.value));
            if (checkbox && checkbox.checked) {
                isActive = true;
            }
        }

        html += `<button class="vibe-chip ${isActive ? 'active' : ''}" data-filter="${def.value}">
