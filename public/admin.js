...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let currentProductVariants = []; // Store variants temporarily while editing
    ...
    adminSidebar.addEventListener('click', async (e) => {
        ...
        if (link) {
            ...
            const pageRenderers = {
                ...
                'page-admin-testimonials': renderAdminTestimonialsPage,
            };
            ...
            if (renderFunction) {
                ...
                if (pageId === 'page-discovery') {
        loadDiscoveryConfig(); // <--- This fills the form with data from Firebase
    }
            }
        }
    });
...
function renderVariantList() {
    ...
    currentProductVariants.forEach((v, index) => {
        const div = document.createElement('div');
        div.className = 'flex justify-between items-center bg-white border p-2 rounded shadow-sm text-sm';
        div.innerHTML = `
            <div>
                <span class="font-bold text-gray-700">${v.type}:</span> 
                <span class="text-gray-900">${v.value}</span>
                <span class="text-green-600 ml-2 text-xs">(${v.priceMod >= 0 ? '+' : ''}£${parseFloat(v.priceMod).toFixed(2)})</span>
            </div>
            <button type="button" class="text-red-500 hover:text-red-700" onclick="removeVariant(${index})">
...
function setupVariantListeners() {
    ...
    newBtn.addEventListener('click', () => {
        ...
        const priceMod = parseFloat(priceInput.value) || 0;

        if (!type || !value) {
            alert("Please enter both a Type (e.g. Size) and a Value (e.g. Large)");
            return;
        }

        currentProductVariants.push({ type, value, priceMod });
        renderVariantList();
...
function openProductModal(item = null) {
    ...
    // Reset Complex Data Arrays
    currentHamperContents = [];
    currentProductVariants = []; // <--- IMPORTANT: Reset variants

    // 3. Handle Field Visibility (Product vs Component)
    const productSpecificRequiredFields = ['product-stock-input', 'product-category-input'];
    const productFields = ['product-bullets-input', 'product-description-input', 'product-stock-input', 'product-category-input', 'product-tag', 'product-active-toggle', 'product-hamper-toggle', 'hamper-contents-section'];
    
    productFields.forEach(id => {
        const el = document.getElementById(id);
        if (el && el.parentElement) {
            el.parentElement.style.display = isComponentMode ? 'none' : 'block';
        }
        // Handle required attributes
        if (productSpecificRequiredFields.includes(id) && document.getElementById(id)) {
            document.getElementById(id).required = !isComponentMode;
        }
    });

    // 4. Populate Data if Editing (item exists)
    if (item) {
        ...
        // Handle Images
        const images = item.imageUrls || (item.imageUrl1 ? [item.imageUrl1] : []);
        document.getElementById('product-images-input').value = images.join(', ');
        ...
        if (!isComponentMode) {
            ...
            if (item.variants && Array.isArray(item.variants)) {
                currentProductVariants = item.variants;
            }
        }
    } else {
        // Create Mode
    ...
    if (!isComponentMode) {
        ...
        if(hamperSection) hamperSection.style.display = isHamper ? 'block' : 'none';
        
        if (typeof renderHamperContentsList === 'function') renderHamperContentsList();

        // --- NEW: Render Variants ---
        if (typeof renderVariantList === 'function') renderVariantList();
    }

    // 6. Show Modal and setup listeners
    productModal.classList.remove('hidden');
    if (typeof setupVariantListeners === 'function') setupVariantListeners(); // Enable the Add button
}
...
if (createReplacementTarget) {
    ...
    // 1. Build the correct query parameter based on whether it's an email or ID
    const searchParam = isEmail ? `email=${encodeURIComponent(query)}` : `orderId=${encodeURIComponent(query)}`;
    
    // 2. USE the searchParam variable you just created (don't use 'email')
    // and ensure it is joined to 'action=search' with an '&'
    const endpoint = `/api/admin-orders?action=search&${searchParam}`;
    ...
    createOrderForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            const token = await fbAuth.currentUser.getIdToken();
            const response = await fetch('/api/admin-orders?action=create', { method: 'POST', headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
            const result = await response.json();
            if (!response.ok) throw new Error(result.error);
...
// 2. RENDER CONTEXTS (UI Generator)
function renderContextsAdmin(contexts) {
    const container = document.getElementById('contexts-list-container');
    container.innerHTML = '';
    ...
    contexts.forEach((ctx, index) => {
        ...
        div.innerHTML = `
            <div style="display:flex; justify-content:space-between; margin-bottom:10px;">
                ...
                <button onclick="removeContext(${index})" style="color:red; border:none; background:none; cursor:pointer;">Remove</button>
            </div>
            <div class="form-group">
                <label>Context Name (Internal ID):</label>
                <input type="text" class="ctx-id-input" value="${ctx.id || ''}" placeholder="e.g., alcohol">
            </div>
            <div class="form-group">
                <label>Triggers (Keywords that activate this):</label>
                <input type="text" class="ctx-triggers-input" value="${(ctx.triggers || []).join(', ')}" placeholder="wine, beer, gin">
...
// 3. RENDER VIRTUAL TAGS
function renderVirtualTagsAdmin(tagsMap) {
    const tbody = document.getElementById('virtual-tags-table-body');
    tbody.innerHTML = '';

    Object.entries(tagsMap).forEach(([tagName, keywords]) => {
        const tr = document.createElement('tr');
        tr.innerHTML = `
            <td><input type="text" class="vtag-name-input" value="${tagName}" placeholder="Chip Name"></td>
            <td><input type="text" class="vtag-keywords-input" value="${(keywords || []).join(', ')}" placeholder="cheese, crackers"></td>
            <td><button onclick="this.closest('tr').remove()" class="btn-sm btn-danger">X</button></td>
        `;
        tbody.appendChild(tr);
    });
}

// 4. BUTTON LISTENERS (Add New Items) - FIXED VERSION

// --- Context Button ---
const addCtxBtn = document.getElementById('add-context-btn');
if (addCtxBtn) {
    addCtxBtn.addEventListener('click', (e) => {
        ...
        const container = document.getElementById('contexts-list-container');
        if (!container) {
            console.error("❌ CRITICAL: Container 'contexts-list-container' not found!");
            return;
        }

        const div = document.createElement('div');
        div.className = 'context-item-card bg-white p-4 border rounded shadow-sm mb-4 relative';
        div.innerHTML = `
            <div class="flex justify-between items-center mb-3 border-b pb-2">
                <span class="font-bold text-gray-700">New Context</span>
                <button type="button" class="text-red-500 hover:text-red-700 text-sm font-medium delete-ctx-btn">Remove</button>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="form-group">
                    <label class="block text-xs font-bold text-gray-500 uppercase">Internal ID</label>
                    <input type="text" class="ctx-id-input w-full p-2 border rounded text-sm bg-gray-50" placeholder="e.g. alcohol">
                </div>
                <div class="form-group">
                    <label class="block text-xs font-bold text-gray-500 uppercase">Display Name (Optional)</label>
                    <input type="text" class="ctx-name-input w-full p-2 border rounded text-sm" placeholder="e.g. Alcohol">
                </div>
                <div class="form-group md:col-span-2">
                    <label class="block text-xs font-bold text-gray-500 uppercase">Triggers (Keywords)</label>
                    <input type="text" class="ctx-triggers-input w-full p-2 border rounded text-sm" placeholder="wine, beer, gin...">
                    <p class="text-xs text-gray-400 mt-1">If user types ANY of these, show the suggestions below.</p>
                </div>
                <div class="form-group md:col-span-2">
        ...
        div.querySelector('.delete-ctx-btn').addEventListener('click', () => div.remove());
        
        container.appendChild(div);
        console.log("✅ New Context Card Appended");
    });
} else {
    console.error("❌ CRITICAL: Button 'add-context-btn' not found in HTML!");
}

// --- Virtual Tag Button ---
const addTagBtn = document.getElementById('add-virtual-tag-btn');
if (addTagBtn) {
    addTagBtn.addEventListener('click', (e) => {
        ...
        tr.innerHTML = `
            <td class="p-3"><input type="text" class="vtag-name-input w-full p-2 border rounded text-sm font-medium" placeholder="New Chip Name"></td>
            <td class="p-3"><input type="text" class="vtag-keywords-input w-full p-2 border rounded text-sm" placeholder="keywords..."></td>
            <td class="p-3 text-right"><button type="button" class="text-red-500 hover:text-red-700 delete-vtag-btn"><i class="fa-solid fa-trash"></i></button></td>
        `;
        tr.querySelector('.delete-vtag-btn').addEventListener('click', () => tr.remove());
        tbody.appendChild(tr);
    });
}

document.getElementById('add-virtual-tag-btn')?.addEventListener('click', () => {
    const tbody = document.getElementById('virtual-tags-table-body');
    const tr = document.createElement('tr');
    tr.innerHTML = `
        <td><input type="text" class="vtag-name-input" placeholder="New Chip Name"></td>
        <td><input type="text" class="vtag-keywords-input" placeholder="keywords..."></td>
        <td><button onclick="this.closest('tr').remove()" class="btn-sm btn-danger">X</button></td>
    `;
    tbody.appendChild(tr);
});
...
// 5. SAVE FUNCTION
document.getElementById('save-discovery-config-btn')?.addEventListener('click', async () => {
    const contexts = [];
    document.querySelectorAll('.context-item-card').forEach(card => {
        contexts.push({
    ...
    const virtual_tags = {};
    document.querySelectorAll('#virtual-tags-table-body tr').forEach(tr => {
        const name = tr.querySelector('.vtag-name-input').value.trim();
        const keys = tr.querySelector('.vtag-keywords-input').value.split(',').map(s => s.trim()).filter(s => s);
        if (name) virtual_tags[name] = keys;
    });

    try {
        await db.collection('config').doc('discovery_engine').set({ contexts, virtual_tags });
        alert("Discovery Engine Updated!");
    } catch (e) {
        console.error(e);
        alert("Error saving.");
    }
...
function addTestimonialInputs(container, data = {}, index) {
    const div = document.createElement('div');
    div.className = 'testimonial-item form-section p-4 border rounded-md bg-gray-50 mb-4';
    div.dataset.index = index;

    div.innerHTML = `
        <h4 class="font-medium text-sm mb-3 text-gray-600">Testimonial ${index + 1}</h4>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="form-group">
                <label class="block text-xs font-medium text-gray-700">Author</label>
                <input type="text" class="testimonial-author w-full mt-1 px-2 py-1.5 border rounded-md text-sm" value="${data.author || ''}" placeholder="e.g. Jane Doe">
            </div>
            <div class="form-group">
                <label class="block text-xs font-medium text-gray-700">Rating (1-5)</label>
                <input type="number" class="testimonial-rating w-full mt-1 px-2 py-1.5 border rounded-md text-sm" value="${data.rating || 5}" min="1" max="5">
            </div>
        </div>
        <div class="form-group mt-3">
            <label class="block text-xs font-medium text-gray-700">Quote</label>
            <textarea class="testimonial-quote w-full mt-1 px-2 py-1.5 border rounded-md text-sm" rows="2" placeholder="Enter review...">${data.quote || ''}</textarea>
        </div>
        <button type="button" class="btn btn-danger btn-sm remove-testimonial-btn mt-3 text-xs"><i class="fa-solid fa-trash-can"></i> Remove</button>
    `;

    div.querySelector('.remove-testimonial-btn').addEventListener('click', () => div.remove());
    container.appendChild(div);
}

async function renderAdminTestimonialsPage() {
    const form = document.getElementById('testimonials-form');
    const container = document.getElementById('testimonials-container');
    const addBtn = document.getElementById('add-testimonial-btn');
    const saveStatus = document.getElementById('testimonials-save-status');

    if (!form) return;
    container.innerHTML = 'Loading...';
    
    try {
        const data = await fetchAdminAPI('/api/content-manager?page=testimonials');
        container.innerHTML = '';
        (data.testimonials || []).forEach((item, i) => addTestimonialInputs(container, item, i));
        if ((data.testimonials || []).length === 0) addTestimonialInputs(container, {}, 0);
    } catch (e) { container.innerHTML = 'Error loading data.'; }

    addBtn.onclick = () => addTestimonialInputs(container, {}, container.children.length);

    form.onsubmit = async (e) => {
        e.preventDefault();
        saveStatus.textContent = 'Saving...';
        const items = [];
        container.querySelectorAll('.testimonial-item').forEach(div => {
            items.push({
                author: div.querySelector('.testimonial-author').value,
                rating: parseInt(div.querySelector('.testimonial-rating').value),
                quote: div.querySelector('.testimonial-quote').value
            });
        });

        await fetchAdminAPI('/api/content-manager?page=testimonials', {
            method: 'POST',
