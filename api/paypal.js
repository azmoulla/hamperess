...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (action === 'capture-order') {
            ...
            // 1. Actually take the payment (or mock it for testing).
            let captureResult, capture;
            if (process.env.PAYPAL_MOCK === 'true') {
                console.log(`PAYPAL_MOCK is active: Simulating successful capture for order ${paypalOrderId}`);
                captureResult = { status: 'COMPLETED' };
                capture = { id: `MOCK-${Date.now()}`, status: 'COMPLETED' };
            } else {
                captureResult = await capturePayPalOrder(paypalOrderId);
                capture = captureResult?.purchase_units?.[0]?.payments?.captures?.[0];
            }

            // NEW (2026-08-02): a capture can legitimately come back PENDING
            // rather than COMPLETED -- PayPal holds the money for its own
            // fraud/e-check review (status_details.reason: PENDING_REVIEW)
            // and settles it later via webhook. This isn't just a sandbox
            // quirk; it can happen to real customers too. Previously we
            // treated this exactly like a real decline (402, no order),
            // which meant a customer who WAS charged saw "Order Failed" and
            // had nothing to show for it. Now: only a genuinely failed/
            // missing capture (DECLINED, or no capture object at all) is
            // treated as a hard failure -- PENDING still creates the order,
            // just flagged so it can be reconciled when the webhook confirms.
            const captureStatus = capture?.status;
            const isHardFailure = captureResult.status !== 'COMPLETED' || !capture || !['COMPLETED', 'PENDING'].includes(captureStatus);
            ...
            const isPendingReview = captureStatus === 'PENDING';

            // 2. Payment is real (captured or pending PayPal's own review) --
            //    now write the actual order, re-validating stock/discount/
            //    credit authoritatively (same logic the existing Card
            //    checkout uses).
            let orderResult;
            try {
                orderResult = await createOrderTransaction({
                    ...
                    paymentStatus: isPendingReview ? 'pending_review' : 'paid'
                });
            } catch (orderError) {
                // Payment succeeded (or is pending) but the order couldn't be
                // created (e.g. stock ran out in the few seconds between
                // create-order and capture). The customer HAS been charged
                // (or PayPal is holding funds) -- this needs a human to
                // reconcile/refund via the PayPal dashboard, not a silent
                // failure. Surface everything needed for that.
                console.error(`CRITICAL: PayPal payment ${capture.id} (£${capture.amount?.value}, status ${captureStatus}) captured but order creation failed: ${orderError.message}. Manual refund via PayPal may be required.`);
            ...
            // 3. Fire the confirmation email -- but only if the payment has
            //    actually completed. For a pending capture, the webhook
            //    handler (api/paypal-webhook.js) sends the confirmation once
            //    PayPal reports PAYMENT.CAPTURE.COMPLETED, so the customer
            //    isn't told "thank you, it's confirmed" while PayPal could
            //    still decline it.
            if (!isPendingReview) {
                try {
            ...
            //    paid (or has a pending payment) and has an order, so don't
