...
const DEFAULTS = {
    ...
    // ✅ ADDED: This enables 'testimonials' in the whitelist
    testimonials: [],
};

export default async function handler(req, res) {
    const { page } = req.query; // e.g., 'about_us', 'faqs', 'testimonials'

    // Security: Whitelist allowed pages based on the DEFAULTS keys
    ...
    try {
        ...
        if (req.method === 'GET') {
            ...
            if (docSnap.exists) {
                // Special handling for legacy data structures (Delivery Info)
                ...
                // Return the specific default object if DB is empty
        ...
        if (req.method === 'POST') {
            ...
            // ✅ ADDED: Logic to save testimonials
            if (page === 'testimonials') {
                if (!payload.testimonials || !Array.isArray(payload.testimonials)) {
                    return res.status(400).json({ error: 'Invalid data. Requires a testimonials array.' });
                }
                await docRef.set({ testimonials: payload.testimonials });
                return res.status(200).json({ success: true, message: 'Testimonials updated successfully.' });
            }

            // Generic Save for other pages
